...
export const AdminGodModeModal: React.FC<AdminGodModeModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    ...
    <div
      className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-ink/75 backdrop-blur-xs transition-opacity"
      role="dialog"
      aria-modal="true"
    >
      <div className="fixed inset-0" onClick={onClose} aria-hidden="true" />

      <div className="relative z-10 w-full max-w-5xl bg-surface text-ink rounded-t-[24px] sm:rounded-2xl border border-line shadow-2xl flex flex-col max-h-[94vh] sm:max-h-[90vh] overflow-hidden animate-in fade-in-50 slide-in-from-bottom-6 sm:zoom-in-95 duration-200 pt-[max(env(safe-area-inset-top,0px),0.5rem)] sm:pt-0 pb-[max(env(safe-area-inset-bottom,0px),0.5rem)] sm:pb-0">
        <ModalErrorBoundary>
        {/* Top Ribbon Banner */}
        <div className="px-6 py-4 border-b border-line bg-sunken flex items-center justify-between">
          ...
          {/* Modal Header */}
          <div className="flex items-center justify-between px-5 sm:px-6 py-4 border-b border-line bg-surface shrink-0">
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-9 h-9 rounded-xl bg-solid text-on-solid flex items-center justify-center shadow-xs shrink-0">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <h2 className="t-card font-bold truncate">
                    Admin Portal — God Mode
                  </h2>
                  <span className="lg-tag text-[10px] font-bold bg-warn-soft text-warn border border-warn-soft">
                    SYSTEM OVERSEER
                  </span>
                </div>
                <p className="t-meta num mt-0.5 truncate text-[11px]">
                  Administered by: <strong>{currentAdmin?.email || 'jnkpappoe@gmail.com'}</strong>
                </p>
              </div>
            </div>
        ...
        {stats && (
          ...
          {/* Global KPI stats summary */}
          {stats && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5 sm:gap-3 p-3.5 sm:p-5 bg-surface border-b border-line shrink-0">
              <div className="p-3 rounded-xl bg-sunken border border-line">
                <div className="flex items-center justify-between t-eyebrow text-[10px] mb-1">
                  <span>Total Users</span>
                  <Users className="w-3.5 h-3.5 text-accent" />
                </div>
                <span className="text-lg sm:text-xl font-bold font-mono-num num text-ink">{stats.totalUsers}</span>
              </div>
            ...
            <div className="p-3.5 rounded-xl bg-sunken border border-line">
              ...
              <div className="p-3 rounded-xl bg-sunken border border-line">
                <div className="flex items-center justify-between t-eyebrow text-[10px] text-warn mb-1">
                  <span>Pending Payouts</span>
                  <Clock className="w-3.5 h-3.5 text-warn" />
                </div>
                <span className="text-lg sm:text-xl font-bold font-mono-num num text-warn">
            ...
            <div className="p-3.5 rounded-xl bg-sunken border border-line">
              ...
              <div className="p-3 rounded-xl bg-sunken border border-line">
                <div className="flex items-center justify-between t-eyebrow text-[10px] text-pos mb-1">
                  <span>Vaults Capital</span>
                  <Building2 className="w-3.5 h-3.5 text-pos" />
                </div>
                <span className="text-lg sm:text-xl font-bold font-mono-num num text-pos truncate block">
            ...
            <div className="p-3.5 rounded-xl bg-sunken border border-line">
              ...
              <div className="p-3 rounded-xl bg-sunken border border-line">
                <div className="flex items-center justify-between t-eyebrow text-[10px] text-accent mb-1">
                  <span>Fees Retained</span>
                  <DollarSign className="w-3.5 h-3.5 text-accent" />
                </div>
                <span className="text-lg sm:text-xl font-bold font-mono-num num text-accent truncate block">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-6 py-3 border-b border-line bg-sunken">
          ...
          {/* Tab Controls & Search */}
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5 px-4 sm:px-6 py-3 border-b border-line bg-sunken shrink-0">
            <div className="lg-seg">
              <button
                type="button"
                onClick={() => setActiveTab('withdrawals')}
                className={`lg-seg-btn text-xs ${activeTab === 'withdrawals' ? 'active' : ''}`}
              >
                Queue ({withdrawals.filter((w) => w.status === 'pending').length})
              </button>
              <button
                type="button"
                onClick={() => setActiveTab('users')}
                className={`lg-seg-btn text-xs ${activeTab === 'users' ? 'active' : ''}`}
              >
                Users ({users.length})
              </button>
              <button
                type="button"
                onClick={() => setActiveTab('stats')}
                className={`lg-seg-btn text-xs ${activeTab === 'stats' ? 'active' : ''}`}
              >
                Telemetry
              </button>
            </div>
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <div className="flex items-center gap-2">
              <div className="relative flex-1 sm:w-56">
                <Search className="w-3.5 h-3.5 absolute left-3 top-1/2 -translate-y-1/2 text-ink-4 pointer-events-none" />
                <input
                  type="text"
                  placeholder="Search requests or users..."
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="lg-input pl-8 py-1.5 text-xs w-full"
                />
              </div>
              ...
              {activeTab === 'withdrawals' && (
                <select
                  ...
                  className="lg-select text-xs py-1.5 px-2.5"
                >
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-4">
          ...
          {/* Tab Content Area */}
          <div className="p-4 sm:p-6 overflow-y-auto flex-1 space-y-4 overscroll-contain touch-pan-y">
            {isLoading ? (
              <div className="py-16 text-center space-y-3">
                <Loader2 className="w-8 h-8 animate-spin mx-auto text-ink-3" />
                <p className="t-meta text-xs">Accessing system telemetry...</p>
              </div>
            ) : (
              ...
              /* WITHDRAWALS QUEUE */
              filteredWithdrawals.length === 0 ? (
                <div className="py-12 text-center t-meta text-xs">
                  No withdrawal requests found matching current criteria.
                </div>
              ) : (
                <div className="space-y-3">
                  {filteredWithdrawals.map((req) => (
                    <div
                      key={req.id}
                      className="lg-card p-4 space-y-3 shadow-xs"
                    >
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-line pb-2.5">
                        <div>
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {req.isEarlyWithdrawal ? (
                              ...
                              <span className="lg-tag lg-tag-pos text-[10px] font-bold">
                                MATURED (2% FEE)
                              </span>
                            )}
                          </div>
                          <p className="t-meta text-[11px] num mt-0.5">
                            Vault: <strong>{req.goalName}</strong> • Submitted: {safeDate(req.createdAt)}
                          </p>
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                          <span className="font-semibold text-xs text-ink">
                            {req.userName} ({req.userEmail})
                          <span
                            className={`lg-tag text-[10px] font-bold uppercase num ${
                              req.status === 'pending'
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                        <div className="p-3 rounded-lg bg-sunken border border-line space-y-1.5">
                          <div className="flex justify-between">
                            <span className="t-meta text-[11px]">Vault Balance:</span>
                        ...
                        <div className="p-3 rounded-lg bg-sunken border border-line space-y-1">
                          <span className="t-eyebrow text-[10px] block mb-1">
                      ...
                      {req.status === 'pending' && (
                        <div className="flex items-center justify-end gap-2 pt-1">
                          ...
                          <button
                            ...
                            className="lg-btn lg-btn-solid lg-btn-sm text-xs"
                          >
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 gap-2.5 md:hidden">
                  {filteredUsers.map((u) => (
                    <div
                      key={u.id}
                      className="lg-card p-3.5 space-y-2.5"
                    >
                      <div className="flex items-center justify-between gap-2">
                        <div className="min-w-0">
                          <div className="font-semibold text-xs text-ink truncate">{u.username}</div>
                          <div className="t-meta text-[11px] truncate">{u.email}</div>
                        </div>
                        <select
                          value={u.role}
                          onChange={(e) => handleRoleChange(u.id, e.target.value as any)}
                          className="lg-select text-xs py-1 px-2 shrink-0"
                        >
                          {req.status}
                        </span>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                      ...
                      <div className="grid grid-cols-3 gap-1.5 p-2 rounded-lg bg-sunken border border-line text-center text-xs">
                        <div>
                          <span className="t-eyebrow text-[9px] block">Profiles</span>
                          <span className="num font-bold text-ink">{u.profilesCount}</span>
                        </div>
                        <div className="flex justify-between text-neg">
                        ...
                        <div>
                          <span className="t-eyebrow text-[9px] block">Txs</span>
                          <span className="num font-bold text-ink">{u.transactionsCount}</span>
                        </div>
                        <div className="flex justify-between pt-1 border-t border-line font-bold text-pos">
                          ...
                          <span className="num text-sm">
                            {req.currency} {safeFmt(req.netPayoutAmount)}
                        <div>
                          <span className="t-eyebrow text-[9px] block">Balance</span>
                          <span className="num font-bold text-ink text-[11px] truncate block">
                    ...
                    {req.status === 'pending' && (
                      ...
                      <div className="flex items-center justify-end gap-2 pt-1 border-t border-line">
                        ...
                        <button
                          ...
                          className="lg-btn lg-btn-quiet lg-btn-sm text-xs text-neg px-2 disabled:opacity-30"
                        >
            ...
            <div className="space-y-2">
              ...
              <div className="bg-surface border border-line rounded-xl overflow-x-auto overscroll-x-contain touch-pan-x shadow-xs">
                ...
                {/* Desktop Table View */}
                <div className="hidden md:block bg-surface border border-line rounded-xl overflow-x-auto shadow-xs">
              ...
              stats && (
                ...
                <div className="p-4 rounded-xl bg-sunken border border-line space-y-3 md:col-span-2">
                  ...
                  <p className="t-meta leading-relaxed">
                    ...
                    <p className="t-meta leading-relaxed text-xs">
                      Paste this Webhook URL into your <strong>Paystack Dashboard &rarr; Settings &rarr; Preferences &rarr; API Keys &amp; Webhooks</strong>. Every customer deposit will be instantly verified and auto-credited to their savings vault.
                    </p>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap">
                      ...
                      <button
                        ...
                        className="lg-btn lg-btn-solid lg-btn-sm shrink-0 text-xs"
                      >
                        {copiedWebhook ? <Check className="w-3.5 h-3.5 mr-1" /> : <Copy className="w-3.5 h-3.5 mr-1" />}
        ...
        <div className="px-6 py-3 border-t border-line bg-surface flex items-center justify-between text-xs t-meta">
          ...
          {/* Modal Footer */}
          <div className="px-5 sm:px-6 py-3 border-t border-line bg-surface flex items-center justify-between text-xs t-meta shrink-0">
            <span className="text-[11px] truncate">Security Protocol: Role Enforcement Guarded</span>
            <button
              onClick={onClose}
              className="lg-btn lg-btn-solid lg-btn-sm text-xs"
            >
      ...
      {approvingId && (() => {
        ...
        return (
          ...
          <div className="fixed inset-0 z-60 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-ink/70 backdrop-blur-xs animate-in fade-in" role="dialog" aria-modal="true">
            <div className="w-full max-w-md lg-card p-5 space-y-4 rounded-t-[24px] sm:rounded-2xl pb-[max(env(safe-area-inset-bottom,0px),1.5rem)] sm:pb-5">
              ...
              <p className="t-body text-xs leading-relaxed">
                Are you sure you want to approve this withdrawal request?
              </p>

              {approvingReq && (
                <div className="p-3.5 rounded-xl bg-sunken border border-line space-y-2 text-xs">
                <div className="p-3 rounded-xl bg-sunken border border-line space-y-1.5 text-xs">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="t-meta text-[11px]">Vault Name:</span>
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button
                  ...
                  className="lg-btn lg-btn-solid lg-btn-sm text-xs disabled:opacity-50"
                >
      ...
      {rejectingId && (
        ...
        <div className="fixed inset-0 z-60 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-ink/70 backdrop-blur-xs animate-in fade-in" role="dialog" aria-modal="true">
          <div className="w-full max-w-md lg-card p-5 space-y-4 rounded-t-[24px] sm:rounded-2xl pb-[max(env(safe-area-inset-bottom,0px),1.5rem)] sm:pb-5">
            <h3 className="t-card font-bold text-neg">Reject Withdrawal Request</h3>
            ...
            <div>
              ...
              <label className="lg-label" htmlFor="godmode-reject-reason">Reason for Rejection</label>
              <input
                id="godmode-reject-reason"
                type="text"
                ...
                className="lg-input text-xs w-full"
              />
            </div>
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button
                ...
                className="lg-btn lg-btn-danger lg-btn-sm text-xs disabled:opacity-50"
              >
                {isProcessing && <Loader2 className="w-3.5 h-3.5 animate-spin" />}
                {isProcessing && <Loader2 className="w-3.5 h-3.5 animate-spin mr-1" />}
